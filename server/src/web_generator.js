...
// 1. FULL-SCALE ALGORAND & ALGOKIT WEB3 DAPP SYNTHESIZER
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .section-title { font-family: 'Outfit', sans-serif; font-size: 30px; font-weight: 800; text-align: center; margin-bottom: 12px; }
    ...
    .step-num { position: absolute; top: 20px; right: 20px; background: rgba(255,255,255,0.05); color: var(--accent); font-family: 'JetBrains Mono', monospace; font-size: 13px; font-weight: 800; padding: 4px 12px; border-radius: 12px; }
    .step-card h3 { font-size: 19px; font-weight: 700; margin-bottom: 12px; color: var(--primary); }
...
<body>
  ...
  <div class="stats-container">
    ...
    <div class="stat-card">
      ...
      <div class="stat-val" id="app-id">#992580122</div>
    </div>
  </div>

  <!-- Application Architecture Steps -->
  <div class="workflow-container">
    <div class="section-title">🛠️ Synthesized Architecture for: "${appName}"</div>
    <div class="section-desc">Automated AI Swarm Mesh Build & Blockchain Smart Contract Integration</div>

    <div class="steps-grid">
      <div class="step-card">
        <span class="step-num">MODULE 1</span>
        <h3>⚙️ Smart Contract & TEAL Bytecode</h3>
        <p>PyTeal approval & clear state programs compiled to Algorand bytecode for <strong>${appName}</strong>.</p>
        <div class="code-block">algokit init --name my-first-algorand-app<br/>algokit project run build</div>
      </div>

      <div class="step-card">
        <span class="step-num">MODULE 2</span>
        <h3>👛 Pera Wallet & TestNet Dispenser</h3>
        <p>Funded deployment account with TestNet ALGO via Dispenser faucet for on-chain state execution.</p>
        <button class="btn-action" style="font-size:12px; padding:8px 16px;" onclick="window.open('https://bank.testnet.algorand.network/', '_blank')">💧 Open TestNet Dispenser</button>
      </div>

      <div class="step-card">
        <span class="step-num">MODULE 3</span>
        <h3>🚀 On-Chain Deployment State</h3>
        <p>Published smart contract to Algorand TestNet blockchain with verified global state schema.</p>
        <div class="code-block">Application ID: 992580122<br/>TxID: TX_9VTSKW6VXSE</div>
      </div>

      <div class="step-card" style="grid-column: span 1 / -1;">
        <span class="step-num">MODULE 4</span>
        <h3>🎉 Real-Time Web Application Execution</h3>
        <p>Active web application build responding live to prompt: <strong>"${appName}"</strong>.</p>
        <div class="code-block">Status: READY | Port: 3005 | Pera Wallet & AlgoKit Active</div>
      </div>
    </div>
  </div>
  ...
  <div class="caller-box">
    ...
    <button class="btn-action" onclick="callSmartContract()">🚀 Execute ${appName} Action (App #992580122)</button>
  </div>
